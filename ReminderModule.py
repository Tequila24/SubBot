...
class ReminderModule:
	...
	def create_reminder(self, peer_id: int, author_id: int, reminder_raw_text: str):
		...
		reminder_text: str = ""
		reminder_time: str = "{0}:{1}:{2}"
		expiration_date: str = ""
		id, hours, minutes, seconds = 0, 0, 0, 0
		...
		match = re.search(r'(\d*) минут(?:ы?)', reminder_raw_text)
		if match:
			minutes = int(match.group(1))
		match = re.search(r'(\d*) секунд(?:ы?)', reminder_raw_text)
		if match:
			seconds = int(match.group(1))
		reminder_time = reminder_time.format(hours, minutes, seconds)
		...
		if response[0] != None:
			...
			id = 1
		print(id, expiration_date, author_id, reminder_text)
		self.dbase.exc("""INSERT INTO 'reminders' VALUES((?), (?), (?), (?), (?));""", (id, expiration_date, author_id, reminder_text, peer_id))
		self.dbase.com()
